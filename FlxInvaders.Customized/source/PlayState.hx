...
class PlayState extends FlxState
{
	...
	override public function create():Void
	{
		...
		if (playerHit)
		{
			FlxG.sound.play("assets/lose.wav");
			mult = 1;
			playerHit = false;
		}
		...
		for (i in 0...numAliens)
		{
			...
			a = new Alien(x, y, colors[Std.int(i / 10)], alienBullets);
			_aliens.add(a);
		}
		...
		for (i in 0...64)
		{
			var x = 32 + 80 * Std.int(i / 16) + (i % 4) * 4;
			var y = FlxG.height - 32 + (Std.int((i % 16) / 4) * 4);
		...
		message = new FlxText(4, 4, TEXT_WIDTH, "");
		add(message);
	}
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (!_player.exists || _aliens.any(a -> a.y >= _player.y - 15))
		{
			deaths++;
			playerHit = true;
			FlxG.resetState();
		...
		message.text = 'Score: $score     Deaths: $deaths';
