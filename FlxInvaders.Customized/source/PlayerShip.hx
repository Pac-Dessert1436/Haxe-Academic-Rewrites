...
{
	private var firingTimer:Float = 0;
	...
	override public function update(elapsed:Float):Void
	{
		...
		var userTouch:FlxTouch = FlxG.touches.getFirst();
		if (userTouch != null)
			handleTouchInput(userTouch, elapsed);
		...
		if (FlxG.keys.justPressed.SPACE || firingTimer > 100)
		{
			firingTimer = 0;
	...
	private function handleTouchInput(touch:FlxTouch, deltaTime:Float):Void
	{
		if (touch.pressed)
		{
			var x0:Float = touch.x;
			velocity.x = if (Math.abs(x - x0) < 0.5) 0 else x > x0 ? -100 : 100;
